...
**The other handlers.** The module firmware compiles in these `goform` names: `aformNetFwHandler`, `aformNetFwUpdateHandler`, `aformFwUpdateProgessHandler` (sic), `aformHandlerSetNetFwUpdate`, `aformHandlerObtainInternetFwUpStatus`, `aformHandlerObtainUsbFwUpStatus`, `aformHandlerObtainUsbBcdList`, `aformHandlerRestartNotify`, `aformHandlerRefreshPage`, `aformHandlerObtainConnStatus`, `aformHandlerConfigureProfileSettings`, `aformHandlerConfigureSSIDList`, `aformHandlerConfigureJbAp`, `aformHandlerConfigureStandByMode`, `aformHandlerConfigureDMPName`, `aformHandlerRestartNetworkSettings`, `aformHandlerTestHandler` and five `aformHandlerSpotify…` ones. GoAhead answers a name it does not know with an "Access Error: Data follows / Form *name* is not defined" page, and a known handler called with no parameters with the empty reply `qwhgpstgrizEndRes`, which is how `python3 jbl_authentics.py fwhandlers <ip>` tells which ones a given web server (bootloader or full firmware) has; it only tries the read‑only ones unless told otherwise, because a bare request still runs the handler.

**How the full firmware reaches the bootloader.** Its own update page has three routes. *Network:* `POST /goform/aformHandlerSetNetFwUpdate` with `readyStatus=0` ("Setting device for file receive"), then `aformNetFwHandler` with `pollStatus=0` every 1.5 s until the reply type is 7 ("System Restarted"), then the browser is sent to `http://<ip>/bl_index.asp`, the bootloader page described above, where the upload happens. *Internet:* `aformHandlerObtainInternetFwUpStatus` with `updateStatus=2` fetches the file from Harman's (long gone) server into RAM ("start bootloaderL2 to update application"). *USB:* `aformHandlerObtainUsbBcdList` lists `.bcd` files on a stick in the module's own USB port and `aformHandlerObtainUsbFwUpStatus` with `BCDUrl=…` starts one. The internet and USB routes report through `aformFwUpdateProgessHandler`, whose `pollStatus=1` is the transfer state and `pollStatus=2` the flash state (same tables as above). `fwstatus` therefore polls both handlers. `python3 jbl_authentics.py fwprepare <ip>` sends the network route's first request to whatever is answering; in the bootloader its effect is untested.

**Observed on a real L16 whose module was stuck in the bootloader.** Before anything was uploaded the state was idle and `pollStatus=2` answered `s.237.149` as the current firmware, i.e. the unit had been on 2.3.7 (the Spotify Connect release). After the first browser upload (the whole `.HUI`, a wrong file) the state became 7 "download failed" and stayed there across restarts. Every upload since, from a browser, `urllib`, a hand‑built HTTP/1.0 request and a WebKit look‑alike, and a 100 KB test, ends with the module accepting the bytes and closing the connection without a reply, the state unchanged, and `pollStatus=2` giving code 5. A USB‑stick update with a genuine 1.2.9 file ran for about 30 minutes (the MCU pushing the 5.9 MB module image over its serial XMODEM link) and rewrote the current‑firmware record to `s.129.149`, so the main board did downgrade itself to 1.2.9 and talk to the module, but the module still came back in the bootloader and no `JBL_L16_Version_Log.txt` appeared on the stick. The "uploaded firmware" field it reports, `s4294967295.538982448.154`, is 0xFFFFFFFF followed by the words at offsets 0x14 and 0x18 of the bCoD header (`0x20203830`, the tail of the build stamp, and 0x9A), so the bootloader had parsed the right header at some point. Both entry routes fail in the same way, which points at the module itself (its flash, or a stuck persistent state) rather than at the file or the transport.

`python3 jbl_authentics.py fwstatus <ip>` sends `1` and `2` to both update handlers and decodes the answers without uploading anything; `--watch 30` keeps polling and prints every change, which is the way to see what the module reports while the main board is pushing an image into it. The manual's USB procedure (file in the root of a USB stick in the top "iPad" USB port, hold Power + Source for five seconds) is the other way in, and needs the same file. The bootloader rejects images meant for another player, so an L8 image should fail validation rather than brick an L16, but do not rely on that.
...
**Container layout.** `HUI ` magic, then at 0x08 the section count (4), at 0x0C the table offset (0x30), at 0x10 the total length, at 0x14 a 32‑bit checksum, and at 0x20 the product tag `JBL_L16`. The table holds four 32‑byte entries: id, four version bytes, offset, size, checksum. Offsets and sizes chain exactly to the end of the file. **The checksum is the two's‑complement negative of the sum of all little‑endian 32‑bit words** (a section whose length is not a multiple of four is zero‑padded), so that the words plus the checksum add up to zero; the file field covers the whole file with itself taken as zero. This matches all four sections and the file field of the copy above, so `python3 jbl_authentics.py fwinfo JBL_L16.HUI` can tell a good copy from a damaged one before it goes anywhere near a speaker.
...
**The module image (`bCoD`, the SDK's `.bcd` file).** Section 2 is a container of its own: magic `bCoD`, a 32‑bit format version 1, the 16‑character build stamp `20131106051708  `, a segment table at 0x30 (per 32‑byte entry: data offset, load address, size, CRC‑32, flags), the marker `DMP 3.x` at 0xB0 and the segment data from 0xB8, padded with 0xFF to exactly 0x5A8000 bytes. The L16 image has three segments: the application (5,158,736 bytes, load address 0x401C0000, DRAM), a 65,260‑byte block with load address 0 (configuration, probably the "CnE Factory" partition) and 706,688 bytes of resources at 0x40700000 (web pages, icons). Each CRC is a plain CRC‑32 (`zlib.crc32`) of the segment's bytes and all three match, so the image the bootloader keeps refusing is internally consistent. The application segment starts with a `0xAAAA5555` boot header naming the bootloader's "second level" partition, which is what the console strings `b1 Bootloader First Level / b2 Bootloader Second Level / ap Application / cf CnE Factory / cu CnE User / pp Persistent Parameters / r<n> Resource` describe as the flash layout.

**What the main board does with the file (from its own strings).** On "Upg Event" it looks for a USB disc, opens `JBL_L16.HUI`, and stores a set of `upgradeConfigbits` in its flash: `fUpgrade_Enable`, `Upgrade_SecCount`, `fHostUpgradeEnable` (itself), `fAirplayUpgradeEnable` (the Wi‑Fi module), `fBTUpgradeEnable`, `fDSPUpgradeEnable`, `fWebpageUpgrade` and `fNeedRebootPowerOn`, so the sequence survives the restarts it needs. The DSP is updated first over SPI ("Erase DSP SPI Flash", "DSP Upg Start"), the Bluetooth chip through its DFU mode ("BT Enter Upg Mode"), then the module: `LAC_CALLBACK_SWITCH_BOOTLOADER_OverUSB` puts it in the bootloader and the image goes across the serial link in XMODEM‑CRC blocks (`LAC_CALLBACK_XMODEM_CRC / ACK / NACK`) until `UpgradeControl.len >= UPG_SecInfo[2].secSize`, then EOT and `Upgrade_EOT_ACK` or `Upgrade_EOT_NACK`, `LAC_UPGRADE_DL_OK` or `LAC_UPGRADE_DL_ERROR`, and finally `Upgrade_Idle_Sucess` or `Upgrade_Idle_Failed`. A missing file is logged as "Brigecoupgrade file size =0". On the real unit that transfer takes about 30 minutes. At the end it writes `JBL_L16_Version_Log.txt` ("Write Sytem Revison Info To Disc") with the running‑time counters, hardware config, build date and the MCU, DSP, Bluetooth and "Airplay" (module) versions, so the absence of that file means the sequence did not reach its last step.
